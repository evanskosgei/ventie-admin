...
const Suspended_creators = () => {
    const [allData, setAllData] = useState([]);
    const [creatorCount, setCreatorCount] = useState(0);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);

    const fetch_suspended_creators = async () => {
        setIsLoading(true);
        setError(null);
        try {
            const { data } = await mtaApi.creators.fetch_creators({ info_type: 3 });
            if (data.status === '200') {
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-screen">
                ...
                <button
                    onClick={fetch_suspended_creators}
    ...
    return (
        <div>
            <PageHeader currentpage="Suspended Creators" activepage="Creators" mainpage="Suspended Creators" />
            <div className="box">
                <div className="box-header lg:flex lg:justify-between">
                    <h5 className="box-title my-auto">List Of Suspended Creators</h5>
                </div>
                <div className="box-body">
                    {allData.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20">
                            ...
                            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No Suspended Creators</h3>
                            <p className="text-gray-500 dark:text-gray-400">There are currently no Suspended creators in the system.</p>
                        </div>
                    )}
                    {allData.length > 0 && (
...
export default Suspended_creators